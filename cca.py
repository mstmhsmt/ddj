...
def run_diffast(container_cmd, original, modified,
    ...
    cca_cmd_path = f'{CCA_HOME}/bin/diffast.opt'
        ...
        try:
            ...
            if view:
                ...
                if os.path.exists(app_path):
                    ...
                    view_cmd = f'open -n {app_path} --args{cache_opt}{files_opt}'
                    if verbose:
                        print(view_cmd)
...
def run_dd(container_cmd, engine, proj_dir, v_good, v_bad,
           build_script='build.sh', test_script='test.sh',
    ...
    if not dry_run:
        if os.path.exists(v_good_dir):
            ...
            print(f'v_good not found: {v_good_dir}')
            return

        if not os.path.exists(v_bad_dir):
            print(f'v_bad not found: {v_bad_dir}')
            return

        if os.path.exists(work_dir):
            print(f'You are about to overwrite "{work_dir}".')
            while True:
    ...
    cca_cmd = f'{CCA_HOME}/ddutil/{engine}.py'
    cca_cmd += f' {cca_proj_dir} {v_good} {v_bad}'
    cca_cmd += f' --build-script {build_script} --test-script {test_script}'
    cca_cmd += f' --proj-id {proj_id}'
    if include:
        cca_cmd += ''.join([f' --include {i}' for i in include])
    if lang:
        cca_cmd += ''.join([f' --lang {i}' for i in lang])
    cca_cmd += f' -a {algo}'
    if shuffle:
        cca_cmd += ' --shuffle'
    if greedy:
    ...
    if max_stmt_level is not None:
        cca_cmd += f' --max-stmt-level {max_stmt_level}'
    if modified_stmt_rate_thresh is not None:
        cca_cmd += f' --modified-stmt-rate-thresh {modified_stmt_rate_thresh}'
    if mem is not None:
        cca_cmd += f' --mem {mem}'

    cca_cmd = f'/bin/bash -c "(time {cca_cmd}) >& {CCA_VAR}/{engine}.log"'

    run_cmd = f'{container_cmd} run --rm -t'
    vol_opt = f' -v "{proj_dir}:{cca_proj_dir}"'
    vol_opt += f' -v "{work_dir}:{CCA_VAR}"'
    installer_path = os.path.join(proj_dir, DEPENDENCIES_INSTALLER)
    if os.path.exists(installer_path):
        vol_opt += f' -v "{installer_path}:{cca_proj_dir}/{DEPENDENCIES_INSTALLER}"'
...
def run_ddplain(container_cmd, proj_dir, v_good, v_bad,
                build_script='build.sh', test_script='test.sh',
                proj_id=None, include=[], lang=[], algo='ddmin',
                shuffle=False, greedy=False, staged=False,
                dry_run=False, devel=False, image=IMAGE_NAME,
                verbose=True, debug=False):
    run_dd(container_cmd, 'ddp', proj_dir, v_good, v_bad,
           build_script=build_script, test_script=test_script,
           proj_id=proj_id, include=include, lang=lang, algo=algo,
           shuffle=shuffle, greedy=greedy, staged=staged,
           dry_run=dry_run, devel=devel, image=image,
           verbose=verbose, debug=debug)


def run_ddjava(container_cmd, proj_dir, v_good, v_bad,
               build_script='build.sh', test_script='test.sh',
               ...
               dry_run=False, devel=False, image=IMAGE_NAME,
               verbose=False, debug=False):
    run_dd(container_cmd, 'ddj', proj_dir, v_good, v_bad,
           build_script=build_script, test_script=test_script,
...
def update(args):
    cmd = f'{args.container_cmd} pull {get_image_name(args.image, devel=args.devel)}'
...
def ddplain(args):
    run_ddplain(args.container_cmd,
                args.proj_dir, args.v_good, args.v_bad,
                args.build_script, args.test_script,
                proj_id=args.proj_id, include=args.include,
                lang=args.lang, algo=args.algo,
...
def main():
    ...
    parser_diffast.add_argument('-c', '--cache', dest='cache',
                                default=DEFAULT_CACHE_DIR, metavar='DIR',
                                type=str, help='result cache directory')
    ...
    parser_ddp = subparsers.add_parser('ddplain',
                                       description=('Delta debugging on changes'
                                                    ' of (plain text) programs'),
                                       parents=[parser],
                                       formatter_class=ArgumentDefaultsHelpFormatter)
    ...
    parser_ddp.add_argument('--lang', type=str, metavar='LANG',
                            action='append', choices=['java', 'python'],
